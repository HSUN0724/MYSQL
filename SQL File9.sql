...
-- select multiple ambiguous columns (aliases)
select passengers.id, pname, ports.id, city
from passengers 
join ports
on portid = ports.id;

select passengers.id as 乘客ID, pname, ports.id as 碼頭ID, city
from passengers 
join ports
on portid = ports.id;

select passengers.id 乘客ID, pname, ports.id 碼頭ID, city
from passengers 
join ports
on portid = ports.id;
...
select *
from passengers
full join ports -- 沒有 full join 這個用法 --
on portid= ports.id;
